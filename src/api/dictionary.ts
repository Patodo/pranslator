...
export async function downloadDictionary(onProgress: (progress: number) => void): Promise<void> {
  const unlisten = await listen<number>('dictionary-download-progress', (event) => {
